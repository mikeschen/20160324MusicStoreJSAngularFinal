...
export class ArtistPipe implements PipeTransform {
  transform(input: Album[], args) {
    ...
    //notice the pipe in task-list.component.ts is taking an array with two values for an argument
    ...
    for (var artist of artistList) {
      console.log("desireed", desiredArtist);
      if (desiredArtist === "all") {
        return input;
      } else if (desiredArtist === artist) {
